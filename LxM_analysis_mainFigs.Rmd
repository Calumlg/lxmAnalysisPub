...
if ("SHAPS_resc" %in% names(coef(mh_fits$best_model)$prolificID)) {
  ...
  newdata <- expand.grid(
    ...
    SHAPS_resc = c(-1, 0, 1),                # Low, mean, and high anhedonia
  ...
  #
  # Slope of postTournRat at a given SHAPS level is:
  #   slope = base_effect + interaction_coef * SHAPS_resc
  # base_effect is the main effect of postTournRat (from the model)
  # interaction_coef is the SHAPS_resc x postTournRat interaction (keep its sign!)
  # SHAPS_resc takes the same values used to build the line plot above (-1.5, 0, 1.5),
  # so the interaction coefficient must be multiplied by that value, not just
  # added/subtracted as a flat amount
  base_effect <- 6.02397
  interaction_coef <- -0.7105  # negative: higher SHAPS -> weaker (smaller) slope

  # Small helper so the slope formula is only written once and applied
  # consistently to each SHAPS_resc value below
  slope_at_shaps <- function(shaps_resc) {
    base_effect + interaction_coef * shaps_resc
  }

  slopes <- data.frame(
    # explicit factor levels (matching SHAPS_resc_factor above) so the bars
    # are ordered Low, Mean, High -- left to right -- exactly like plot2,
    # instead of ggplot's default alphabetical ordering (High, Low, Mean)
    SHAPS_level = factor(c("Low", "Mean", "High"), levels = c("Low", "Mean", "High")),
    Slope = c(slope_at_shaps(-1),  # Low SHAPS
              slope_at_shaps(0),     # Mean SHAPS
              slope_at_shaps(1))   # High SHAPS
  )
  
  # Create bar plot comparing slopes at different SHAPS levels
  plot3 <- ggplot(slopes, aes(x = SHAPS_level, y = Slope, fill = SHAPS_level)) +
    geom_col(width = 0.6) +
    # colors now line up with SHAPS_level's Low/Mean/High order, matching plot2's
    # scale_color_manual (Low = red, Mean = yellow, High = blue)
  ...
  ggsave(
      filename = "shaps_interaction_combined.png",  # change path/name as needed
      plot = comb_intr_plot,
      dpi = 400,
      width = 12,
      height = 8,
      units = "in",
      bg = "white"   # avoids a transparent background when pasted into Word
  )
